...
SELECT 
	...
	SUM(SalesQuantity) AS 'Resumo Quantidade Vendida'
FROM
	FactSales
GROUP BY
	channelKey
...
SELECT 
	BrandName AS 'Marca',
	COUNT(BrandName) AS 'Quantidade Produtos'
FROM 
	DimProduct
GROUP BY
	BrandName
...
SELECT
	Gender AS 'Sexo',
	COUNT(Gender) AS 'Quantidade Total de Clientes',
...
SELECT
	...
	COUNT(Education) AS 'Quantidade Total de Clientes',
	AVG(YearlyIncome) AS 'Media Salarial'
FROM 
	DimCustomer
WHERE
