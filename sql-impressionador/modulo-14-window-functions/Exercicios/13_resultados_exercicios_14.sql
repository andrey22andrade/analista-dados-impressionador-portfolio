...
SELECT 
	 *, 
	 SUM(Quantidade_Vendida) OVER() AS 'Qtd Total Produtos', 
	 SUM(Quantidade_Vendida) OVER(PARTITION BY Marca) AS 'Qtd Total Por 
	Marca' 
FROM vwProdutos 
ORDER BY Marca 
GO
...
SELECT 
	 *, 
	 ...
	 FORMAT(1.0*(SUM(Quantidade_Vendida) OVER(PARTITION BY 
	Marca))/SUM(Quantidade_Vendida) OVER(), '0.00%') 
FROM vwProdutos 
ORDER BY Marca 
GO
